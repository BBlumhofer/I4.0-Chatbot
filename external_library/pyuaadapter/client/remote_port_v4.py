...
class RemotePort(CoupleMixin, RemoteComponent):
    """ Represents a remote port (specialized remote component) using the skill v4 node set. """

    @property
    def active_port(self) -> bool:
        """ Returns whether the remote port is active. """
        return self.attributes['IsActive'].value

    @property
    def own_rfid_tag(self) -> str:
        """ Returns the own RFID tag of the remote port. """
        return self.attributes['OwnRfidTag'].value
    ...
    @property
    def closed(self) -> str:
        """ Returns the boolean if magnet is closed """
        return self.monitoring['MagnetsClosed'].value

    # @property
    # def position(self) -> list[float]:
    #     """ Returns the position (X, Y, Z) of this remote port. """
    #     return self.monitoring['Position']
